...
class ViewsTestCase(TestCase):
    ...
    def test_client_photo_view(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(self.client1.get_client_photo_data() in response.context['clients'])
...
class FormsTestCase(TestCase):
    def test_client_form(self):
        ...
        Test create image object, positive validation form, negative validation form
        """
...
class ApiTestCase(APITestCase):
    def setUp(self):
        ...
        """
        super().setUp()
        self.client_id = randint(0, 100000)
        Client.objects.create(
            id=self.client_id,
            first_name='FirstName',
            last_name='LastName',
        ...
        self.client_entity = Client.objects.get(id=self.client_id)

    def test_client_photo_view(self):
        """
        Check client photo view
        Check that created client is found and check her id
        """
        response = self.client.get(reverse('api_photo_list'), format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(len(response.data) > 0)
        self.assertEqual(response.data[0]['id'], self.client_entity.id)
    ...
    def test_client_photo_like_view(self):
        ...
        self.assertEqual(updated_client.likes, 1)
